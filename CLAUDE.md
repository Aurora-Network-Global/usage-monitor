...
- **Dynamic** (`Aurora-OpenAIRE-Usage-Dashboard-dynamic.html`): has no `RAW` literal at all. An async `loadRAW()` fetches the same 10 CSVs from `../data/` by **fixed filename** (not a pattern — see "CSV exports in data/" below), decodes them (UTF-16, see below), loads them into an in-browser DuckDB via DuckDB-Wasm, runs SQL, and assembles a `RAW` object of the same shape plus a `meta` block (period, load timestamp, per-portal row counts) that the page renders from instead of hand-typed prose. Refreshing this dashboard just means overwriting the same 10 filenames in `data/` — everything recomputes on next load, no code edit. Any file may be absent (`loadTable()` treats a 404 as "no data for this dataset," not an error) — including a whole portal's worth of files, which the rendering code (see `PORTALS` below) handles by hiding that portal's UI rather than showing it zeroed. See `docs/data.md` for the exact CSV→SQL→`RAW` mapping, the single-portal behavior, and the DuckDB/CSV quirks it took to get an exact match against the static file's numbers.

Key pieces in the script, top to bottom:
- `RAW` — the embedded dataset described above.
- `PORTALS_ALL` / `PORTALS` — `PORTALS_ALL` is the full two-portal config (key, display name, URL, CSS color variable, role blurb) this report was built for; `PORTALS` is `PORTALS_ALL` filtered down to whichever of those actually have data loaded (`RAW[p.key] && RAW[p.key].length`). Everything downstream iterates `PORTALS`, so a deployment missing one portal's data just gets one portal's worth of UI — see `docs/data.md`.
...
The `data/Export _ *.csv` files are raw Matomo exports (Channel Type, City, Country, Main metrics, Page URLs — each split by portal) and are the source material `RAW` was built from (baked in for the static file; fetched live for the dynamic one). They are **UTF-16 encoded**; reading them with a UTF-8-assuming tool (`cat`, `grep`, naive `csv` libraries) will show garbled/spaced-out text. Decode as UTF-16 (e.g. Python `open(path, encoding='utf-16')`, or `iconv -f utf-16`) before parsing.

Filenames are **fixed** (e.g. `Export _ Main metrics _ CONNECT.csv`) rather than Matomo's own default, which embeds the export's date range and changes on every re-export — the dynamic loader fetches these 10 exact names, not a pattern, so a fresh export must be renamed to match before it's dropped in. Full list and refresh procedure: `docs/data.md`, `README.md`.
