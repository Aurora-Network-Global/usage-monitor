...
- **Inline tooltips** — plain `title` attributes (same mechanism `.flagpill` already used for the member/partner/automated-traffic hints), styled with a dotted underline + `cursor:help` (`.substats .k[title]`, `thead th[title]`) so the hint is discoverable without hunting. Currently used on every "Bounce rate"/"Bounce" label to explain the metric.
